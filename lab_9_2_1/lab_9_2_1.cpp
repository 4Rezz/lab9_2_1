...
void Sort(Student* p, const int N)
{
	...
	for (int i0 = 0; i0 < N - 1; i0++) {
		for (int i1 = 0; i1 < N - i0 - 1; i1++) {
			int avar_one = GPA(p[i1]);
			int avar_two = GPA(p[i1 + 1]);
			if ((avar_one > avar_two)
				||
				(avar_one == avar_two &&
...
int GPA(Student p) {
	...
	{
	case KN:
		avarage = (p.fiz + p.mat + p.prog) / 3;
		break;
	case ME:
		avarage = (p.fiz + p.mat + p.pedagog) / 3;
		break;
	case IF:
		avarage = (p.fiz + p.mat + p.chis_metod) / 3;
		break;
	case FI:
		avarage = (p.fiz + p.mat + p.pedagog) / 3;
		break;
